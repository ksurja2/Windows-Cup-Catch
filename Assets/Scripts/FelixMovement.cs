...
public class FelixMovement : MonoBehaviour
{
    ...
    private TargetController _target; //reference to target class
    ...
    public bool breaktime; //indicates whether the game is paused
	...
	void Update()
    {
        ...
        //when the game is paused, turn off all forces
        if (breaktime == true)
        {
            MasterForce = 0;
        }
        //when the game is running, turn on forces
        if (breaktime == false)
        {
            MasterForce = 1.0f;
        }
        ...
        //user can adjust error augmentation 
        ...
        //calibration (in Cupcatch , this will change to handangles[3])
/*        if (Input.GetKeyDown("5"))
    ...
    //the "Get" functions below use custom functions from the ConnectionSetter class
